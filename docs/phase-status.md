...
**테스트**: 백엔드 622건 · 프론트 74건 · 타입체크 0 errors / 0 warnings · 빌드 성공
...
- `GET /api/v1/auth/departments`는 미인증으로 부서명을 노출한다. 가입 폼의 드롭다운 때문이며
  의도된 노출이다.
- **브라우저 렌더 확인 14건 미확인.** API 흐름은 Cloudflare 터널을 통해 18건을 실측했다
  (가입 → 중복 → 대소문자 우회 차단 → 승인 전 로그인 → 상태 누출 없음 → 중복 사번·없는 직급
  거부 → 승인 → 재승인 409 → 로그인 성공 → 거절 → 재신청 → ACTIVE 거절 409). 하지만
  `/signup`과 승인 모달의 **화면은 아직 사람이 본 적이 없다.** 특히 모달의 Esc·백드롭 클릭
  닫기와 744px 미만 반응형이 미확인이다. 목록은
  [`manual-scenarios.md`](manual-scenarios.md)의 "가입·승인 신규" 절.
- `/admin/users`의 상태 필터 `Select`는 자체 placeholder("선택하세요", `value=""`)를 렌더하므로
  우리가 넣은 "전체"(역시 빈 값)와 겹친다. 기능은 정상이나 첫 표시가 "선택하세요"일 수 있다.

### 구현 중 잡은 결함

테스트·타입체크·빌드가 모두 통과하는 상태에서 나온 것들이라 기록해 둔다.

- **대소문자만 다른 이메일이 가입 중복 가드를 둘 다 우회했다.** Postgres의 `=`도 unique
  인덱스도 대소문자를 구분하고 Pydantic의 `EmailStr`은 도메인만 소문자화한다. 남이 신청해 둔
  대기 계정 옆에 사실상 같은 주소의 행을 심어 관리자 승인을 가로챌 수 있었다. 스키마 경계의
  `field_validator`로 고쳤고, 운영 DB에 기존 혼합 대소문자 행은 0개였다. 코드 리뷰가
  실제 요청을 보내 발견했다 — 단위 테스트로는 잡히지 않았다.
- **유니크 위반이 500이었다.** Agent는 5xx를 재시도하므로 절대 성공할 수 없는 요청에 재시도
  루프가 걸린다. `delete_entity` 패턴대로 409로 바꿨다.
- **시드의 대기 계정이 운영 DB에 만들어지지 않았다.** `_seed_users`의 early-return 안쪽에
  넣었기 때문이다. 빈 스키마로 시작하는 테스트는 통과했고, 운영 DB를 눈으로 보고서야 발견했다.
  `CLAUDE.md`에 규칙으로 올렸다.
